...
from games_odds.save_games_into_db import SaveOddsIntoDb
...
class TestingGetOddsFromCsvAndStoreIntoDbForLink0(TestCase, SaveOddsIntoDb, CombineOddsWithItsMatch):
    def setUp(self):
    ...
    def test_03_check_ids_for_tag_span_link_csv_is_in_db(self):
        get_url = WilliamHillCsvLinks.objects.get(url_name=str('link_0'))
        self.assertEqual(str('get_match_odds_link_0.csv'), get_url.ids_for_tag_span_link_csv)
    ...
    def test_04_check_that_ids_for_tag_span_link_0_csv_is_stored_in_tag_name_span_attr_ids_folder(self):
        ...
        self.assertEqual(base_dir + "/games_odds/williamHillFiles/get_match_odds/get_match_odds_link_0.csv", ids_for_tag_span_link_0)

    def test_05_check_both_csv_files_ids_for_tag_span_link_0_is_the_same(self):
        base_dir = settings.BASE_DIR
        get_testing_files = base_dir + "/games_odds/tests/webScrapingTests/testingFiles/get_match_odds_link_0.csv"
    ...
    def test_06_store_list_of_matches_into_db(self):
        get_ids_for_tag_span_link_0 = self.get_games_from_csv_file(str('link_0'))
        get_ids_for_tag_span_link_0_list = self.store_csv_files_into_list(get_ids_for_tag_span_link_0)
        store_tag_span_link_0_list = WilliamHillOdds0(games=get_ids_for_tag_span_link_0_list, url_game_link_id=1)
